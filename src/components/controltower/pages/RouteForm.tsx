...
interface SpaceSharingInfo {
  id: string; // 唯一标识
  company: string; // 船公司代码
  internalRouteCode: string; // 内部航线代码
}
...
interface RouteFormData {
  ...
  routeName: string;
  alliance: string;
  spaceSharing: string[]; // 保留用于兼容性，将被spaceSharingList替代
  spaceSharingList: SpaceSharingInfo[]; // 新的共舱方信息列表
...
const RouteForm: React.FC = () => {
    ...
    try {
      ...
      for (let i = 0; i < formData.spaceSharingList.length; i++) {
        ...
        if (!spaceSharing.company) {
          Message.error(`共舱方 ${i + 1} 必须选择船公司`);
          return;
        }
        if (!spaceSharing.internalRouteCode?.trim()) {
          Message.error(`共舱方 ${i + 1} 必须输入内部航线代码`);
          return;
        }
      }
      ...
      setFormData(prev => ({
        ...
        spaceSharing: spaceSharingCompanies, // 保持兼容性
        spaceSharingList: formData.spaceSharingList // 新的数据结构
      }));
  ...
  const addSpaceSharing = () => {
    if (formData.spaceSharingList.length >= 20) {
      Message.warning('最多只能添加20组共舱方信息');
      return;
    }
    ...
    Message.success('已添加新的共舱方信息');
  };

  /**
   * 删除指定的共舱方信息
   * @param spaceSharingId 共舱方ID
   */
  const removeSpaceSharing = (spaceSharingId: string) => {
    ...
    Message.success(`已删除${companyName}的共舱方信息`);
  };
  ...
  const updateSpaceSharing = (spaceSharingId: string, field: keyof SpaceSharingInfo, value: string) => {
    ...
    if (field === 'company' && value) {
      ...
      if (isDuplicate) {
        ...
        Message.warning(`船公司 ${companyLabel} 已存在，请选择其他船公司`);
        return;
      }
    }
  ...
  /**
   * 获取可用的船公司选项（排除已选择的）
   * @param currentSpaceSharingId 当前共舱方ID，用于排除自身
   * @returns 可用的船公司选项
   */
  ...
  const [supplementFields, setSupplementFields] = useState<{
    [key: string]: { // scheduleId-portCode
      ...
      transitPorts: TransitPort[]; // 中转港列表
                    ...
                    <Row gutter={16}>
                        ...
                        <Form.Item
                          label={
                            <span style={{ fontWeight: 500, color: '#1d2129' }}>
                              船公司 <span style={{ color: '#f53f3f' }}>*</span>
                            </span>
                          }
                      ...
                      <Col span={12}>
                        <Form.Item
                          label={
                            <span style={{ fontWeight: 500, color: '#1d2129' }}>
                              内部航线代码 <span style={{ color: '#f53f3f' }}>*</span>
                            </span>
                          }
                          field={`spaceSharingList.${index}.internalRouteCode`}
                          rules={[
                            { required: true, message: '请输入内部航线代码' },
                            { 
                              validator: (value) => {
                                if (!value) return Promise.resolve();
                                if (!/^[A-Z0-9]{2,20}$/.test(value)) {
                                  return Promise.reject('内部航线代码应为2-20位大写字母和数字组合');
                                }
  ...
  const SupplementDrawer = () => {
    ...
    /**
     * 获取中转港的显示名称（带序号）
     * @param index 中转港在数组中的索引
     * @returns 格式化的中转港名称，如"第一中转港"、"第二中转港"
     */
    const getTransitPortLabel = (index: number): string => {
      const numbers = ['一', '二', '三', '四', '五', '六', '七', '八', '九', '十'];
      if (index < numbers.length) {
        return `第${numbers[index]}中转港`;
      }
      return `第${index + 1}中转港`;
    };
        ...
        <Form form={form} layout="vertical">
          ...
          <div style={{ marginTop: 24 }}>
            <div style={{ 
              ...
              <Title heading={6} style={{ margin: 0 }}>中转港</Title>
              <Button
                type="primary"
                icon={<IconPlus />}
