...
/**
 * 代客订舱页面组件
 * 用于处理运营版本控制塔的代客订舱申请功能
 */
const BookingSubmissionForClient: React.FC<BookingSubmissionProps> = () => {
  ...
  // 模拟企业客户数据（状态为正常的企业）
  const clientOptions = [
    { id: '1', name: '上海华润物流有限公司', status: 'active' },
    { id: '2', name: '深圳中远海运集装箱运输有限公司', status: 'active' },
    { id: '3', name: '青岛港国际股份有限公司', status: 'active' },
    { id: '4', name: '宁波舟山港集团有限公司', status: 'active' },
    { id: '5', name: '天津港股份有限公司', status: 'active' },
    { id: '6', name: '广州港集团有限公司', status: 'active' },
    { id: '7', name: '厦门港务控股集团有限公司', status: 'active' },
    { id: '8', name: '大连港集团有限公司', status: 'active' }
  ];
  
  // 过滤客户选项（模糊搜索）
  const filteredClientOptions = clientOptions.filter(client =>
  ...
  const vesselVoyageDropdownRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      // 检查船名航次下拉框
      if (vesselVoyageDropdownRef.current && !vesselVoyageDropdownRef.current.contains(event.target as Node)) {
  ...
  // 处理委托客户选择
  const handleClientSelect = (client: { id: string; name: string; status: string }) => {
          ...
          <div className="mb-4">
            <div className="flex items-center gap-2 text-sm text-gray-600">
              ...
              {showCargoTypeDropdown && (
                <div ref={cargoTypeDropdownRef} className="relative">
                  <Select
                    value={cargoType}
              ...
              {showVesselVoyageDropdown && (
                <div ref={vesselVoyageDropdownRef} className="relative">
                  <Select
                    value={vesselVoyage}
...
export default BookingSubmissionForClient;